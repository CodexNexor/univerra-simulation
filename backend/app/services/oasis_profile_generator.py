...
Converts entities from local graph into Agent Profile format required by the OASIS simulation platform

Optimizations:
1. Calls local graph retrieval functions to enrich node information
...
logger = get_logger('univerra.oasis_profile')
...
class OasisProfileGenerator:
    ...
    Converts entities from local graph into Agent Profiles required for OASIS simulation

    Optimization features:
    1. Calls local graph retrieval to get richer context
    ...
    COUNTRIES = [
        "Global", "US", "UK", "Japan", "Germany", "France",
    ...
    def generate_profile_from_entity(
        ...
        """
        Generate OASIS Agent Profile from local graph entity

        Args:
            entity: local graph entity node
        ...
        effective_use_llm = use_llm and not bool((entity.attributes or {}).get("synthetic"))
        ...
        profile_data["persona"] = self._add_future_time_awareness(profile_data.get("persona", ""))
    ...
    def _add_future_time_awareness(self, persona: str) -> str:
        marker = "[Simulated time:"
        addition = (
            " During the simulation, this agent treats any post prefix such as "
            f"{marker} ...] as the current in-world time and updates reactions, urgency, "
            "memory, and expectations according to that future timestamp."
        )
        persona = (persona or "").strip()
        if "simulated time" in persona.lower():
            return persona
        return (persona + addition).strip()
    ...
    def _search_graph_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Uses local graph hybrid search to get rich information related to an entity

        Local graph does not have a built-in hybrid search API, so edges and nodes must be searched separately and results merged.
        ...
        try:
            ...
            edge_search = tools.search_graph(self.graph_id, entity.name, limit=20, scope="edges")
            node_search = tools.search_graph(self.graph_id, entity.name, limit=12, scope="nodes")

            results["facts"] = list(dict.fromkeys(edge_search.facts))
            results["node_summaries"] = list(dict.fromkeys(
                node.get("summary", "") or f"Related entity: {node.get('name', '')}"
                for node in node_search.nodes
            ...
            logger.warning(f"Local graph retrieval failed ({entity.name}): {error}")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Rich information retrieved via Local graph hybrid search
        """
        context_parts = []
        ...
        # 4. Use local graph retrieval for richer information
        ...
        if graph_results.get("facts"):
            ...
            if new_facts:
                context_parts.append("### Facts Retrieved via Graph Search\n" + "\n".join(f"- {f}" for f in new_facts[:15]))

        if graph_results.get("node_summaries"):
            context_parts.append("### Related Nodes Retrieved via Graph Search\n" + "\n".join(f"- {s}" for s in graph_results["node_summaries"][:10]))
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 6. Try extracting partial info from content
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate concise, realistic personas for public opinion simulation, maximizing fidelity to existing real-world conditions. Focus on behavior drivers, memory, stance, language style, and how the persona would react when posts include simulated future timestamps. Must return valid JSON format. All string values must not contain unescaped newlines. Use natural English."
        return base_prompt
...
1. bio: Social media bio, within 120 characters
2. persona: Simulation-ready persona description (single paragraph, 220-420 words), must include:
   ...
   - Positions and opinions (attitude toward topics, content that may provoke/move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memory (important part of persona: describe this individual's connection to events and existing actions/reactions)
   - Future-time behavior (how they update beliefs and react when simulated posts are explicitly timestamped in the future)
3. age: Age number (must be integer)
4. gender: Must be in English: "male" or "female"
5. mbti: MBTI type (e.g., INTJ, ENFP)
6. country: Country or region in English (e.g., "Global" or "United States")
...
- Persona should live inside the simulated future clock when posts include "[Simulated time: ...]" context
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an institutional/group entity, maximizing fidelity to existing real-world conditions.
...
1. bio: Official account bio, within 120 characters, professional and appropriate
2. persona: Simulation-ready account profile description (single paragraph, 220-420 words), must include:
   - Institutional basic info (formal name, nature, background, main functions)
   ...
   - Institutional memory (important part of persona: describe this institution's connection to events and existing actions/reactions)
   - Future-time behavior (how the account changes communication when simulated posts carry future timestamps)
3. age: Fixed at 30 (virtual age for institutional account)
4. gender: Fixed as "other" (institutional accounts use other to indicate non-personal)
5. mbti: MBTI type to describe account style, e.g., ISTJ for strict and conservative
6. country: Country or region in English (e.g., "Global" or "United States")
...
- Institutional account communication must match its identity positioning"""
    ...
    def _generate_profile_rule_based(
        ...
        if entity_attributes.get("synthetic"):
            topics = []
            for candidate in ["AI", "Python", "Hiring", "Careers", "Backend", "Security", "Interviewing", "Projects"]:
                if candidate.lower() in f"{entity_name} {entity_summary} {entity_type}".lower():
                    topics.append(candidate)
            if not topics:
                topics = ["Career Growth", "Industry Signals", "Skills"]

            stance = str(entity_attributes.get("stance", "neutral"))
            perspective = str(entity_attributes.get("perspective", "")).strip()
            persona = entity_summary or f"{entity_name} represents a focused scenario perspective inside the simulation."
            if perspective:
                persona += f" This persona usually argues from a {perspective} point of view."
            if stance == "opposing":
                persona += " They actively challenge easy optimism and emphasize downside risks, weak evidence, and automation pressure."
            elif stance == "supportive":
                persona += " They usually encourage action, highlight paths forward, and look for practical leverage."
            elif stance == "observer":
                persona += " They compare tradeoffs carefully and do not automatically agree with the majority view."

            return {
                "bio": entity_summary[:120] if entity_summary else f"{entity_name} perspective for the simulation.",
                "persona": persona,
                "age": 32 if "manager" in entity_type_lower or "recruiter" in entity_type_lower else 28,
                "gender": "other",
                "mbti": "INTJ" if "analyst" in entity_type_lower or "mentor" in entity_type_lower else "ENTJ",
                "country": "Global",
                "profession": entity_type.replace("_", " "),
                "interested_topics": topics[:4],
            }
    ...
    def set_graph_id(self, graph_id: str):
        """Set graph ID for local graph retrieval"""
        self.graph_id = graph_id
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Graph ID for local graph retrieval of richer context
        ...
        # Set graph_id for local graph retrieval
        if graph_id:
            self.graph_id = graph_id
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        user_char vs description differences:
