...
from .scenario_agent_expander import ScenarioAgentExpander
from .reddit_research import RedditResearchService

logger = get_logger('univerra.simulation')
...
class SimulationManager:
    ...
    Core features:
    1. Read entities from local graph and filter
    ...
    def prepare_simulation(
        ...
        Steps:
        1. Read and filter entities from local graph
        ...
        try:
            ...
            if progress_callback:
                progress_callback("reading", 0, "Opening local graph...")
            ...
            filtered = ScenarioAgentExpander().expand(
                filtered=filtered,
            ...
            # Pass graph_id to enable richer local graph retrieval context
            ...
            profiles = generator.generate_profiles_from_entities(
                ...
                graph_id=state.graph_id,  # Pass graph_id for local graph retrieval
            ...
            augmented_document_text = document_text
            reddit_research_result = RedditResearchService().research(
                query=simulation_requirement,
                scenario_context=document_text[:4000],
            )
            if not reddit_research_result.error:
                reddit_path = os.path.join(sim_dir, "reddit_research.json")
                with open(reddit_path, "w", encoding="utf-8") as file:
                    json.dump(reddit_research_result.to_dict(), file, ensure_ascii=False, indent=2)
                augmented_document_text = (
                    (document_text or "")
                    + "\n\n## Fresh Reddit Research\n"
                    + reddit_research_result.to_context_text()
                ).strip()
            ...
            sim_params = config_generator.generate_config(
                ...
                graph_id=state.graph_id,
                simulation_requirement=simulation_requirement,
                document_text=augmented_document_text,
                entities=filtered.entities,
                profiles=profiles,
    ...
    def list_simulations(
        ...
        if os.path.exists(self.SIMULATION_DATA_DIR):
            for sim_id in os.listdir(self.SIMULATION_DATA_DIR):
                ...
                if state:
                    project_matches = project_id is None or state.project_id == project_id
                    user_matches = user_id is None or state.user_id == user_id
                    if project_matches and user_matches:
    ...
    def get_run_instructions(self, simulation_id: str) -> Dict[str, str]:
        ...
        return {
            ...
            "instructions": (
                f"1. Activate conda environment: conda activate Univerra\n"
