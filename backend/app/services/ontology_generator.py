...
API 1: Analyzes text content and generates entity and relationship type definitions suitable for social simulation
"""
...
- We need to simulate reactions from all parties and information propagation paths in opinion events
...
Your 10 entity types must include the following tiers:

A. **Fallback types (must include, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual person. Used when a person doesn't fit other more specific person types.
   - `Organization`: Fallback type for any organization. Used when an organization doesn't fit other more specific types.

B. **Specific types (8, designed based on text content)**:
   - Design more specific types for key roles appearing in the text
...
- Various people appear in text, such as "teachers", "bystanders", "some netizen"
- If there's no specific type match, they should fall under `Person`
- Similarly, small organizations, temporary groups should fall under `Organization`
...
**Individual (fallback)**:
- Person: Any individual (used when not fitting above specific types)
...
- Organization: Any organization (used when not fitting above specific types)
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # Truncate if text exceeds 50K characters (only affects LLM input, not graph building)
...
4. All entity types must be real-world agents that can speak up, not abstract concepts
5. Attribute names cannot use reserved words like name, uuid, group_id; use full_name, org_name instead
6. Preserve explicit dates, times, deadlines, and named real-world periods when they appear in the source material
"""
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # local graph limits: max 10 custom entity types, max 10 custom edge types
        MAX_ENTITY_TYPES = 10
        MAX_EDGE_TYPES = 10
        ...
        if fallbacks_to_add:
            ...
            if current_count + needed_slots > MAX_ENTITY_TYPES:
                ...
                # Remove from end (keep more important specific types at the beginning)
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines = [
            ...
            'Auto-generated by Univerra for social media opinion simulation',
            '"""',
            '',
