...
Step 2: local graph entity reading and filtering, OASIS simulation preparation and execution (fully automated)
"""
...
from ..services.scenario_agent_expander import ScenarioAgentExpander
from ..services.simulation_manager import SimulationManager, SimulationStatus
from ..services.simulation_runner import SimulationRunner, RunnerStatus
from ..services.user_context import apply_user_goal_context, strip_user_goal_context
from ..services.user_store import UserStore, UserStoreUnavailable
from ..utils.logger import get_logger
from ..utils.rate_limiter import check_rate_limit, get_client_ip
from ..models.project import ProjectManager

logger = get_logger('univerra.api.simulation')
...
def _request_identity(user=None) -> str:
    if user:
        return user.get("user_id") or get_client_ip()
    return get_client_ip()


def _save_user_simulation(user_id: str, simulation_id: str, metadata: dict):
    if not user_id:
        return
    try:
        UserStore.save_simulation(user_id, simulation_id, metadata)
    except UserStoreUnavailable as exc:
        logger.warning(f"Could not save simulation metadata to MongoDB: {exc}")
...
def create_simulation():
    ...
    Request (JSON):
        {
            ...
            "graph_id": "univerra_xxxx",    // Optional, fetched from project if not provided
    ...
    try:
        ...
        blocked = check_rate_limit(
            "simulation:create",
            _request_identity(user),
            Config.RATE_LIMIT_SIMULATION_CREATE_PER_HOUR,
            3600,
        )
        if blocked:
            return blocked
        ...
        if project.user_id and (not user or project.user_id != user.get("user_id")):
            return jsonify({
                "success": False,
        ...
        if user:
            _save_user_simulation(user["user_id"], state.simulation_id, {
                "project_id": project_id,
...
def prepare_simulation():
    ...
    2. Read and filter entities from local graph
    ...
    try:
        ...
        blocked = check_rate_limit(
            "simulation:prepare",
            _request_identity(user),
            Config.RATE_LIMIT_SIMULATION_PREPARE_PER_HOUR,
            3600,
        )
        if blocked:
            return blocked
        ...
        if state.user_id and (not user or state.user_id != user.get("user_id")):
            return jsonify({
                "success": False,
                "error": "You do not have access to this simulation"
            }), 403
        ...
        original_simulation_requirement = project.simulation_requirement or ""
        if not original_simulation_requirement:
            return jsonify({
                "success": False,
                "error": "Project missing simulation requirement description (simulation_requirement)"
            }), 400
        simulation_requirement = apply_user_goal_context(original_simulation_requirement, user)
        ...
        try:
            ...
            filtered_preview = ScenarioAgentExpander().expand(
                filtered=filtered_preview,
        ...
        if user:
            _save_user_simulation(user["user_id"], simulation_id, {
                "project_id": state.project_id,
                "project_name": project.name,
                "graph_id": state.graph_id,
                "status": state.status.value,
                "simulation_requirement": original_simulation_requirement,
                "goal_context_applied": simulation_requirement != original_simulation_requirement,
            })
        ...
        def run_prepare():
            try:
                ...
                if user:
                    ...
                    temporal = config.get("temporal_forecast_config", {})
                    dashboard = config.get("calibration_dashboard", {})
                    _save_user_simulation(user["user_id"], simulation_id, {
                        "project_id": state.project_id,
                        "project_name": project.name,
                        "graph_id": state.graph_id,
                        ...
                        "forecast_start_at": temporal.get("forecast_start_at"),
                        "forecast_end_at": temporal.get("forecast_end_at"),
                        "simulation_hours": time_config.get("total_simulation_hours"),
                        "quality_score": dashboard.get("overall_quality_score"),
                    })
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    try:
        ...
        if config:
            temporal_config = config.get("temporal_forecast_config", {})
            calibration_profile = config.get("calibration_profile", {})
            ensemble_config = config.get("ensemble_config", {})
            calibration_dashboard = config.get("calibration_dashboard", {})
            signal_plan = config.get("real_world_signal_plan", {})
            response_data["summary"] = {
                "total_agents": len(config.get("agent_configs", [])),
                "simulation_hours": config.get("time_config", {}).get("total_simulation_hours"),
                "forecast_start_at": temporal_config.get("forecast_start_at"),
                "forecast_end_at": temporal_config.get("forecast_end_at"),
                "timezone": temporal_config.get("timezone"),
                "confidence_label": calibration_profile.get("confidence_label"),
                "confidence_score": calibration_profile.get("confidence_score"),
                "recommended_ensemble_runs": ensemble_config.get("recommended_runs"),
                "outcome_paths_count": len(config.get("outcome_probability_table", [])),
                "counterfactual_controls_count": len(config.get("counterfactual_controls", [])),
                "real_world_signal_sources_count": len(signal_plan.get("sources", [])),
                "overall_quality_score": calibration_dashboard.get("overall_quality_score"),
                "real_world_prediction_score": calibration_dashboard.get("real_world_prediction_score"),
                "initial_posts_count": len(config.get("event_config", {}).get("initial_posts", [])),
                "scheduled_events_count": len(config.get("event_config", {}).get("scheduled_events", [])),
...
def get_simulation_config(simulation_id: str):
    ...
    try:
        ...
        manager = SimulationManager()
        state = manager.get_simulation(simulation_id)
        if state and state.user_id and (not user or state.user_id != user.get("user_id")):
            return jsonify({
                "success": False,
                "error": "You do not have access to this simulation"
            }), 403
...
def generate_profiles():
    ...
    Request (JSON):
        {
            "graph_id": "univerra_xxxx",     // Required
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "ensemble_run_index": 0,                // Optional: seeded run variant
            "scenario_variant": "baseline_path",    // Optional: scenario variant from ensemble_config
            "enable_graph_memory_update": false,   // Optional: whether to dynamically update Agent activities to local graph memory
    ...
    About enable_graph_memory_update:
        - When enabled, all Agent activities (posting, commenting, liking, etc.) are updated to local graph in real-time
    ...
    try:
        ...
        blocked = check_rate_limit(
            "simulation:start",
            _request_identity(user),
            Config.RATE_LIMIT_SIMULATION_START_PER_HOUR,
            3600,
        )
        if blocked:
            return blocked
        ...
        ensemble_run_index = data.get('ensemble_run_index', 0)
        scenario_variant = data.get('scenario_variant', 'baseline_path')
        ...
        try:
            ensemble_run_index = max(0, int(ensemble_run_index or 0))
        except (ValueError, TypeError):
            return jsonify({
                "success": False,
                "error": "ensemble_run_index must be a valid non-negative integer"
            }), 400
        scenario_variant = str(scenario_variant or "baseline_path")[:80]
        ...
        run_state = SimulationRunner.start_simulation(
            ...
            graph_id=graph_id,
            ensemble_run_index=ensemble_run_index,
        ...
        if state.user_id:
            project = ProjectManager.get_project(state.project_id)
            config = manager.get_simulation_config(simulation_id) or {}
            _save_user_simulation(state.user_id, simulation_id, {
                "project_id": state.project_id,
                "project_name": project.name if project else "",
                "graph_id": state.graph_id,
                "status": state.status.value,
                "simulation_requirement": strip_user_goal_context(
                    config.get("simulation_requirement", project.simulation_requirement if project else "")
                ),
                "platform": platform,
                "max_rounds": max_rounds,
                "ensemble_run_index": ensemble_run_index,
        ...
        response_data['ensemble_run_index'] = ensemble_run_index
        response_data['scenario_variant'] = scenario_variant
