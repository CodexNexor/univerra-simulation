...
logger = get_logger('univerra.api.report')
...
def _save_report_history(user_id: str, simulation_id: str, metadata: dict):
    if not user_id:
        return
    try:
        UserStore.save_simulation(user_id, simulation_id, metadata)
    except UserStoreUnavailable as exc:
        logger.warning(f"Could not save report metadata to MongoDB: {exc}")
...
def generate_report():
    ...
    try:
        ...
        blocked = check_rate_limit(
            ...
            Config.RATE_LIMIT_REPORT_GENERATE_PER_HOUR,
            3600,
        )
        if blocked:
            return blocked
        ...
        simulation_requirement = strip_user_goal_context(config.get("simulation_requirement") or project.simulation_requirement)
        simulation_requirement = apply_user_goal_context(simulation_requirement, user)
        ...
        def run_generate():
            try:
                ...
                if report.status == ReportStatus.COMPLETED:
                    if state.user_id:
                        _save_report_history(state.user_id, simulation_id, {
...
@report_bp.route('/<report_id>/evaluation', methods=['GET'])
def get_report_evaluation(report_id: str):
    """Get deterministic report quality/evidence evaluation."""
    try:
        evaluation = ReportManager.get_evaluation(report_id)
        if not evaluation:
            return jsonify({
                "success": False,
                "error": f"Report evaluation not found: {report_id}"
            }), 404
        ...
        logger.error(f"Failed to get report evaluation: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
