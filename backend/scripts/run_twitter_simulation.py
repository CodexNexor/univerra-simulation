...
This script reads parameters from a config file to execute simulations, enabling full automation.

Features:
- After simulation completes, does not immediately close environment; enters command wait mode
...
# Load the .env file from the project root directory (contains LLM_API_KEY and other configs)
...
from app.utils.llm_base import normalize_openai_base_url


from simulation_policy import (
    PlatformRuntimePolicy,
    build_create_post_actions,
    compute_simulation_seed,
    get_recommended_semaphore,
)
...
class TwitterSimulationRunner:
    ...
    def __init__(
        ...
        self.config["ensemble_run_index"] = ensemble_run_index
        self.config["scenario_variant"] = scenario_variant
        self.simulation_dir = os.path.dirname(config_path)
        self.wait_for_commands = wait_for_commands
        self.seed = compute_simulation_seed(self.config, "twitter")
        self.env = None
        self.agent_graph = None
        self.ipc_handler = None
        self.runtime_policy = PlatformRuntimePolicy(self.config, "twitter")
    ...
    def _create_model(self):
        ...
        Use configuration from project root .env file (highest priority):
        - LLM_API_KEY: API key
        ...
        if llm_base_url:
            os.environ["OPENAI_API_BASE_URL"] = normalize_openai_base_url(llm_base_url)
    ...
    def _get_active_agents_for_round(
        ...
        current_hour: int,
        round_num: int,
        current_absolute_hour: float = None
    ) -> List:
        """
        ...
        """
        minutes_per_round = self.config.get("time_config", {}).get("minutes_per_round", 30)
        return self.runtime_policy.select_active_agents(
            env,
            current_hour,
            round_num,
            minutes_per_round,
            current_absolute_hour=current_absolute_hour,
        )
    ...
    async def run(self, max_rounds: int = None):
        ...
        self.env = oasis.make(
            ...
            semaphore=get_recommended_semaphore(len(self.config.get("agent_configs", []))),
        )
        ...
        initial_posts = self.runtime_policy.annotate_initial_posts(initial_posts)

        if initial_posts:
            print(f"Execute initial events ({len(initial_posts)}initial posts)...")
            initial_actions, prepared_posts = build_create_post_actions(
                self.env,
                initial_posts,
                ManualAction,
                ActionType.CREATE_POST,
            ...
            self.runtime_policy.observe_posts(prepared_posts)

            if initial_actions:
                await self.env.step(initial_actions)
                print(f"  Published {len(prepared_posts)} initial posts")
        ...
        for round_num in range(total_rounds):
            ...
            scheduled_posts = self.runtime_policy.get_due_scheduled_posts(round_num, minutes_per_round)
            if scheduled_posts:
                scheduled_actions, prepared_posts = build_create_post_actions(
                    self.env,
                    scheduled_posts,
                    ManualAction,
                    ActionType.CREATE_POST,
                ...
                if scheduled_actions:
                    ...
                    self.runtime_policy.observe_posts(prepared_posts)
                    print(f"  Triggered {len(prepared_posts)} scheduled posts")

            # Get Agents activated this round
            active_agents = self._get_active_agents_for_round(
                self.env, simulated_hour, round_num, simulated_minutes / 60.0
            )
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Seeded ensemble run index for multi-run comparison'
    )
    parser.add_argument(
        '--scenario-variant',
        type=str,
        default='baseline_path',
        help='Scenario variant name for seeded ensemble comparison'
    )
    ...
    runner = TwitterSimulationRunner(
        ...
        ensemble_run_index=args.ensemble_run_index,
...
def setup_signal_handlers():
    """
    Set up signal handlers to ensure graceful exit when receiving SIGTERM/SIGINT.
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            ...
            # Only force exit if a repeated signal is received
            print("Force exiting...")
            sys.exit(1)
