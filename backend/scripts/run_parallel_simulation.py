...
# Script is located at backend/scripts/ directory
...
async def run_twitter_simulation(
    ...
    runtime_policy = PlatformRuntimePolicy(config, "twitter")
    ...
    result.env = oasis.make(
        ...
        semaphore=get_recommended_semaphore(len(config.get("agent_configs", []))),
    )
    ...
    initial_posts = runtime_policy.annotate_initial_posts(initial_posts)
    ...
    if initial_posts:
        ...
        runtime_policy.observe_posts(prepared_posts)
    ...
    for round_num in range(total_rounds):
        ...
        scheduled_posts = runtime_policy.get_due_scheduled_posts(round_num, minutes_per_round)
        if scheduled_posts:
            ...
            if scheduled_actions:
                ...
                if action_logger:
                    for post in prepared_posts:
                        ...
                        total_actions += 1
                runtime_policy.observe_posts(prepared_posts)
                log_info(f"Triggered {len(prepared_posts)} scheduled Twitter posts")

        active_agents = runtime_policy.select_active_agents(
            result.env,
            simulated_hour,
            round_num,
            minutes_per_round,
            current_absolute_hour=simulated_minutes / 60.0,
        )
        ...
        runtime_policy.observe_actions(actual_actions)
...
async def run_reddit_simulation(
    ...
    runtime_policy = PlatformRuntimePolicy(config, "reddit")
            ...
            if scheduled_actions:
                ...
                if action_logger:
                    for post in prepared_posts:
                        ...
                        total_actions += 1
                runtime_policy.observe_posts(prepared_posts)
                log_info(f"Triggered {len(prepared_posts)} scheduled Reddit posts")
...
async def main():
    ...
    # Create shutdown event at the start of main function to ensure the entire program can respond to shutdown signals
    ...
    config["ensemble_run_index"] = args.ensemble_run_index
    config["scenario_variant"] = args.scenario_variant
    simulation_dir = os.path.dirname(args.config) or "."
    wait_for_commands = not args.no_wait

    base_seed = compute_simulation_seed(config, "parallel")
    ...
    log_manager.info(f"Deterministic seed: {base_seed}")
