        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Run Simulation" and Step5 "Deep Interaction" must be launched during runtime and do not support history replay</span>
            </div>
          </div>
        </div>
      </Transition>
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Use debounced state transition to prevent rapid flickering
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Small delay before executing to avoid rapid switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
.gradient-overlay {
  ...
  bottom: 0;
  background:
    linear-gradient(to right, rgba(5, 5, 5, 0.95) 0%, transparent 15%, transparent 85%, rgba(5, 5, 5, 0.95) 100%),
    linear-gradient(to bottom, rgba(5, 5, 5, 0.95) 0%, transparent 20%, transparent 80%, rgba(5, 5, 5, 0.95) 100%);
  pointer-events: none;
}
...
.file-tag.txt { background: rgba(255, 255, 255, 0.08); color: #757575; }
...
.modal-progress.not-started { color: #9CA3AF; background: rgba(255, 255, 255, 0.05); }
