<template>
  <div class="graph-panel" v-reveal>
...
const renderGraph = () => {
  ...
  // Process edge data, calculate edge counts and indices between node pairs
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // This ensures all edges distribute in the same frame of reference without overlapping
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const getLinkPath = (d) => {
    ...
    // More edges means greater proportional offset
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      linkLabelBg.attr('fill', 'rgba(20,20,20,0.85)')
      linkLabels.attr('fill', '#DDD')
  ...
  const linkLabelBg = linkGroup.selectAll('rect')
    ...
    .on('click', (event, d) => {
      ...
      linkGroup.selectAll('path').attr('stroke', 'rgba(255,255,255,0.3)').attr('stroke-width', 1.5)
      linkLabelBg.attr('fill', 'rgba(20,20,20,0.85)')
      linkLabels.attr('fill', '#FFFFFF')
      // Highlight corresponding edge
      link.filter(l => l === d).attr('stroke', '#3498db').attr('stroke-width', 3)
      d3.select(event.target).attr('fill', 'rgba(52, 152, 219, 0.2)')
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .on('click', (event, d) => {
      ...
      node.attr('stroke', 'rgba(255,255,255,0.2)').attr('stroke-width', 2.5)
      linkGroup.selectAll('path').attr('stroke', 'rgba(255,255,255,0.3)').attr('stroke-width', 1.5)
    ...
    .on('mouseenter', (event, d) => {
      if (!selectedItem.value || selectedItem.value.data?.uuid !== d.rawData.uuid) {
        d3.select(event.target).attr('stroke', 'rgba(255,255,255,0.8)').attr('stroke-width', 3)
      }
    })
    .on('mouseleave', (event, d) => {
      if (!selectedItem.value || selectedItem.value.data?.uuid !== d.rawData.uuid) {
        d3.select(event.target).attr('stroke', 'rgba(255,255,255,0.2)').attr('stroke-width', 2.5)
      }
    })
