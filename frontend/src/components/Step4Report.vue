<template>
  <div class="report-panel" v-reveal>
    <!-- Main Split Layout -->
    <div class="main-split-layout">
      ...
      <div class="left-panel report-style" ref="leftPanel">
        <div v-if="reportOutline" class="report-content-wrapper">
          ...
          <div class="report-header-block">
            ...
            <button class="chatgpt-toggle-btn" @click="showTimeline = !showTimeline">
              <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2">
                <polyline points="22 12 18 12 15 21 9 3 6 12 2 12"></polyline>
              </svg>
              <span>{{ showTimeline ? 'Hide Process Timeline & Metrics' : 'View Agent Process & Metrics' }}</span>
            </button>
          </div>
      ...
      <div class="right-panel" ref="rightPanel" v-show="showTimeline">
...
watch(isComplete, (newVal) => {
  if (newVal) {
    setTimeout(goToInteraction, 5000)
  }
})
  ...
  try {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (quotesMatch) {
        ...
        // Fallback: match > "text" or typographic quote variants
        if (!quoteMatches) {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "\u95ee\u9898X：" or "\u95ee\u9898X:" (legacy format)
      // 2. "1. " or "\n1. " (number+dot, legacy format)
      let matches = []
      let match

      // Try legacy question-label format first
      const cnPattern = /(?:^|[\r\n]+)\u95ee\u9898(\d+)[：:]\s*/g
...
.generated-content :deep(.md-h2) { font-size: 20px; border-bottom: 1px solid rgba(255, 255, 255, 0.1); padding-bottom: 8px; }
...
.right-panel {
  ...
  /* Functional palette (dark mode status-based) */
  --wf-border: rgba(255, 255, 255, 0.1);
  --wf-divider: rgba(255, 255, 255, 0.05);

  --wf-active-bg: rgba(255, 255, 255, 0.05);
  --wf-active-border: rgba(255, 255, 255, 0.2);
  --wf-active-dot: #FFFFFF;
  --wf-active-text: #FFFFFF;

  --wf-done-bg: transparent;
  --wf-done-border: rgba(255, 255, 255, 0.1);
  --wf-done-dot: #10B981;

  --wf-muted-dot: rgba(255, 255, 255, 0.2);
  --wf-todo-text: rgba(255, 255, 255, 0.4);
}
...
.log-content::-webkit-scrollbar-thumb { background: var(--bg-primary); border-radius: 2px; }
