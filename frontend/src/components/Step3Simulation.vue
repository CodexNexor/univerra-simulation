<template>
  <div class="simulation-panel" v-reveal>
    <!-- Top Control Bar -->
...
const doStartSimulation = async () => {
  ...
  try {
    const params = {
      ...
      ensemble_run_index: props.ensembleRunIndex || 0,
      scenario_variant: props.scenarioVariant || 'baseline_path'
    }
    ...
    addLog(`Ensemble seed variant: run ${params.ensemble_run_index}, ${params.scenario_variant}`)
...
// Auto-finish transition
watch(phase, (newPhase) => {
  if (newPhase === 2 && !isGeneratingReport.value) {
    handleNextStep() // Auto-navigate to Report when complete
  }
})

// Scroll log to bottom (keeping logic but removed UI element)
...
.platform-status.twitter .platform-icon { color: var(--text-main); }
.platform-status.reddit .platform-icon { color: var(--text-main); }
...
.breakdown-item.twitter { color: var(--text-main); }
.breakdown-item.reddit { color: var(--text-main); }
...
.timeline-item.twitter .marker-dot { background: var(--bg-primary); }
.timeline-item.reddit .marker-dot { background: var(--bg-primary); }
.timeline-item.twitter .timeline-marker { border-color: var(--text-main); }
.timeline-item.reddit .timeline-marker { border-color: var(--text-main); }
...
.badge-post { background: rgba(255, 255, 255, 0.08); color: var(--text-main); border-color: rgba(255, 255, 255, 0.1); }
.badge-comment { background: rgba(255, 255, 255, 0.05); color: var(--text-muted); border-color: rgba(255, 255, 255, 0.05); }
.badge-action { background: rgba(255, 255, 255, 0.02); color: var(--text-muted); border: 1px solid rgba(255, 255, 255, 0.05); }
.badge-meta { background: rgba(0, 0, 0, 0.2); color: var(--text-muted); border: 1px dashed rgba(255, 255, 255, 0.1); }
