<template>
  <div class="env-setup-panel" v-reveal>
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          <p class="description">
            Create new simulation instance and fetch simulation world parameter template
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          <p class="description">
            Leveraging context, automatically extract entities and relationships from the knowledge graph, initialize simulated individuals, and endow them with unique behaviors and memories based on reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="description">
            LLM intelligently configures world time flow, recommendation algorithms, individual activity windows, posting frequency, event triggers, and other parameters based on simulation requirements and reality seeds
          </p>
            ...
            <div class="config-block">
              <div class="config-grid">
                ...
                <div class="config-item">
                  ...
                  <span class="config-item-value compact">{{ simulationConfig.temporal_forecast_config?.timezone || simulationConfig.environment_context?.timezone || '-' }}</span>
                </div>
                <div class="config-item">
                  <span class="config-item-label">Forecast Start</span>
                  <span class="config-item-value compact">{{ formatDateTime(simulationConfig.temporal_forecast_config?.forecast_start_at) }}</span>
                </div>
                <div class="config-item">
                  <span class="config-item-label">Forecast End</span>
                  <span class="config-item-value compact">{{ formatDateTime(simulationConfig.temporal_forecast_config?.forecast_end_at) }}</span>
                </div>
                <div class="config-item">
                  <span class="config-item-label">Confidence</span>
                  <span class="config-item-value compact">{{ simulationConfig.calibration_profile?.confidence_label || '-' }} {{ simulationConfig.calibration_profile?.confidence_score ?? '' }}</span>
                </div>
              </div>
              ...
              <div v-if="simulationConfig.temporal_forecast_config?.phase_windows?.length" class="phase-windows">
                <div class="phase-window" v-for="phase in simulationConfig.temporal_forecast_config.phase_windows" :key="phase.name">
                  <div class="phase-topline">
                    <span class="phase-name">{{ phase.name?.replaceAll('_', ' ') }}</span>
                    <span class="phase-hours">H{{ phase.start_hour }}-{{ phase.end_hour }}</span>
                  </div>
                  <div class="phase-time">{{ formatDateTime(phase.start_at) }} → {{ formatDateTime(phase.end_at) }}</div>
                  <p class="phase-focus">{{ phase.expected_focus }}</p>
                </div>
              </div>

              <div v-if="simulationConfig.calibration_dashboard" class="quality-dashboard">
                <div class="quality-card">
                  <span class="quality-label">Overall Quality</span>
                  <span class="quality-value">{{ toScore10(simulationConfig.calibration_dashboard.overall_quality_score) }}/10</span>
                </div>
                <div class="quality-card">
                  <span class="quality-label">Real-World Prediction</span>
                  <span class="quality-value">{{ toScore10(simulationConfig.calibration_dashboard.real_world_prediction_score) }}/10</span>
                </div>
                <div class="quality-card">
                  <span class="quality-label">Agent Diversity</span>
                  <span class="quality-value">{{ toScore10(simulationConfig.calibration_dashboard.agent_diversity_score) }}/10</span>
                </div>
                <div class="quality-card">
                  <span class="quality-label">Data Coverage</span>
                  <span class="quality-value">{{ toScore10(simulationConfig.calibration_dashboard.data_coverage_score) }}/10</span>
                </div>
              </div>
            </div>

            <div v-if="simulationConfig.outcome_probability_table?.length" class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">Outcome Probability Table</span>
                <span class="config-block-badge">{{ simulationConfig.ensemble_config?.recommended_runs || '-' }} runs recommended</span>
              </div>
              <div class="outcome-list">
                <div v-for="row in simulationConfig.outcome_probability_table" :key="row.outcome" class="outcome-row">
                  <div class="outcome-main">
                    <span class="outcome-name">{{ row.outcome }}</span>
                    <span class="outcome-reason">{{ row.why }}</span>
                  </div>
                  <span class="outcome-prob">{{ row.probability_percent }}%</span>
                </div>
              </div>
            </div>

            <div v-if="simulationConfig.counterfactual_controls?.length" class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">Counterfactual Controls</span>
                <span class="config-block-badge">{{ simulationConfig.counterfactual_controls.length }} tests</span>
              </div>
              <div class="counterfactual-grid">
                <div v-for="control in simulationConfig.counterfactual_controls" :key="control.id" class="counterfactual-card">
                  <span class="counterfactual-title">{{ control.label }}</span>
                  <p class="counterfactual-effect">{{ control.expected_effect }}</p>
                </div>
              </div>
            </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          ...
          <div v-if="simulationConfig?.event_config" class="orchestration-content">
            ...
            <div v-if="simulationConfig.event_config.scheduled_events?.length" class="initial-posts-section">
              <span class="box-label">Future Scheduled Events ({{ simulationConfig.event_config.scheduled_events.length }})</span>
              <div class="posts-timeline">
                <div v-for="event in simulationConfig.event_config.scheduled_events" :key="event.event_id" class="timeline-item">
                  <div class="timeline-marker scheduled"></div>
                  <div class="timeline-content">
                    <div class="post-header">
                      <span class="post-role">{{ event.phase || 'future phase' }}</span>
                      <span class="post-agent-info">
                        <span class="post-id">H{{ event.trigger_hour }}</span>
                        <span class="post-username">{{ formatDateTime(event.simulated_at) }}</span>
                      </span>
                    </div>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">Univerra auto-plans to simulate <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours, each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of real-time flow</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                ...
                <div class="numeric-input-row">
                  <div class="numeric-field">
                    <span class="numeric-label">Rounds</span>
                    <input
                  ...
                  <div class="numeric-help">
                    <span class="help-label">Available Range</span>
                    <span class="help-value">10 to {{ autoGeneratedRounds }}</span>
                    <button class="recommended-btn" @click="customMaxRounds = 40">Use 40</button>
                  </div>
                </div>
              </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For first runs, we strongly recommend switching to ‘Custom Mode’ with fewer rounds for a quick preview and reduced error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
...
const actionRequestSent = ref(false)
...
const handleStartSimulation = () => {
  ...
  const firstVariant = simulationConfig.value?.ensemble_config?.run_variants?.[0]
  if (firstVariant) {
    params.ensembleRunIndex = firstVariant.run_index || 0
    params.scenarioVariant = firstVariant.scenario_variant || 'baseline_path'
    addLog(`Using ensemble seed variant: run ${params.ensembleRunIndex}, ${params.scenarioVariant}`)
  }

  emit('next-step', params)
}

const broadcastFrontendActionRequest = () => {
  if (!simulationConfig.value || !autoGeneratedRounds.value || actionRequestSent.value) return

  const recommendedRounds = Math.min(40, autoGeneratedRounds.value)
  window.dispatchEvent(new CustomEvent('univerra-action-required', {
    detail: {
      type: 'start-dual',
      title: 'Custom Simulation Details',
      message: 'The backend is ready for your approval. You can launch the dual-world simulation now or switch to fewer rounds for a faster preview.',
      simulationId: props.simulationId,
      autoRounds: autoGeneratedRounds.value,
      recommendedRounds,
      allowCustomRounds: true,
      actionLabel: 'Start Dual Processing'
    }
  }))
  actionRequestSent.value = true
}

const handleFrontendAuthorizeAction = (event) => {
  const detail = event?.detail || {}
  if (detail.type !== 'start-dual') return
  if (phase.value < 4) return

  if (detail.useCustomRounds && detail.maxRounds) {
    useCustomRounds.value = true
    customMaxRounds.value = Number(detail.maxRounds)
  } else {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (res.data.expected_entities_count) {
        ...
        addLog(`Read ${res.data.expected_entities_count} entities from the Univerra knowledge graph`)
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          ...
          if (data.summary.forecast_start_at && data.summary.forecast_end_at) {
            addLog(`  ├─ Forecast window: ${formatDateTime(data.summary.forecast_start_at)} → ${formatDateTime(data.summary.forecast_end_at)} (${data.summary.timezone || 'UTC'})`)
          }
          if (data.summary.confidence_label) {
            addLog(`  ├─ Confidence: ${data.summary.confidence_label} ${data.summary.confidence_score ?? ''}`)
          }
          if (data.summary.recommended_ensemble_runs) {
            addLog(`  ├─ Ensemble recommendation: ${data.summary.recommended_ensemble_runs} seeded runs`)
          }
          if (data.summary.overall_quality_score) {
            addLog(`  ├─ Quality dashboard: ${(data.summary.overall_quality_score * 10).toFixed(1)}/10 overall, ${(data.summary.real_world_prediction_score * 10).toFixed(1)}/10 real-world`)
          }
          addLog(`  ├─ Outcome paths: ${data.summary.outcome_paths_count || 0}`)
          addLog(`  ├─ Counterfactual controls: ${data.summary.counterfactual_controls_count || 0}`)
          addLog(`  ├─ Real-world signal sources: ${data.summary.real_world_signal_sources_count || 0}`)
          addLog(`  ├─ Initial posts: ${data.summary.initial_posts_count}`)
          addLog(`  ├─ Scheduled future events: ${data.summary.scheduled_events_count || 0}`)
        ...
        broadcastFrontendActionRequest()
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        if (res.data.summary) {
          ...
          if (res.data.summary.forecast_start_at && res.data.summary.forecast_end_at) {
            addLog(`  ├─ Forecast window: ${formatDateTime(res.data.summary.forecast_start_at)} → ${formatDateTime(res.data.summary.forecast_end_at)} (${res.data.summary.timezone || 'UTC'})`)
          }
          if (res.data.summary.confidence_label) {
            addLog(`  ├─ Confidence: ${res.data.summary.confidence_label} ${res.data.summary.confidence_score ?? ''}`)
          }
          if (res.data.summary.recommended_ensemble_runs) {
            addLog(`  ├─ Ensemble recommendation: ${res.data.summary.recommended_ensemble_runs} seeded runs`)
          }
          if (res.data.summary.overall_quality_score) {
            addLog(`  ├─ Quality dashboard: ${(res.data.summary.overall_quality_score * 10).toFixed(1)}/10 overall, ${(res.data.summary.real_world_prediction_score * 10).toFixed(1)}/10 real-world`)
          }
          addLog(`  ├─ Initial posts: ${res.data.summary.initial_posts_count}`)
          addLog(`  ├─ Scheduled future events: ${res.data.summary.scheduled_events_count || 0}`)
          addLog(`  └─ Outcome paths: ${res.data.summary.outcome_paths_count || 0}, counterfactuals: ${res.data.summary.counterfactual_controls_count || 0}, signal sources: ${res.data.summary.real_world_signal_sources_count || 0}`)
        }
...
.badge.success { background: rgba(46, 125, 50, 0.2); color: #4CAF50; }
.badge.processing { background: #FF5722; color: #FFF; }
.badge.pending { background: rgba(255, 255, 255, 0.05); color: var(--text-muted); }
.badge.accent { background: rgba(33, 150, 243, 0.2); color: #2196F3; }
