<template>
  <div class="workbench-panel" v-reveal>
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
        ...
        <div class="card-content">
          ...
          <div v-if="errorMessage && currentPhase <= 0" class="inline-error">
            {{ errorMessage }}
          </div>
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
        ...
        <div class="card-content">
          <p class="description">
            Based on the generated ontology, automatically chunk documents and build the Univerra knowledge graph, extracting entities and relationships while forming temporal memory and community summaries.
          </p>
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 2, 'completed': currentPhase >= 2 }">
        ...
        <div class="card-content">
          <p class="description">Graph construction is complete. Automatically proceeding to simulation environment setup...</p>
          <button
            class="action-btn"
...
const canEnterEnvironmentSetup = computed(() =>
  props.currentPhase >= 2 && Boolean(props.projectData?.project_id) && Boolean(props.projectData?.graph_id)
)

const broadcastFrontendActionRequest = () => {
  if (!canEnterEnvironmentSetup.value || actionRequestSent.value) return

  window.dispatchEvent(new CustomEvent('univerra-action-required', {
    detail: {
      type: 'enter-environment-setup',
      title: 'Environment Setup Ready',
      message: 'Graph building is complete. The backend is waiting for approval to enter simulation environment setup.',
      actionLabel: 'Enter Environment Setup'
    }
  }))
...
const handleFrontendAuthorizeAction = async (event) => {
  ...
  if (detail.type !== 'enter-environment-setup') return
  if (!canEnterEnvironmentSetup.value || creatingSimulation.value) return
  await handleEnterEnvSetup()
}
...
watch(canEnterEnvironmentSetup, (ready) => {
  if (ready) {
    broadcastFrontendActionRequest()
  }
}, { immediate: true })

// Auto-scroll logs (removed from UI, keeping logic for safety)
